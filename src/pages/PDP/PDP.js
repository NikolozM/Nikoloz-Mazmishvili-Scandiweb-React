...
if (data){
  for ( let i = 0 ; i < data.product.gallery.length ; i ++ ){
      images.push(<img key={i} src={data.product.gallery[i]}
      onClick={()=> setMainImage(data.product?.gallery[i])}/>);
    }
  description = data.product.description?.replace(new RegExp('<[^>]*>', 'g'), '');
...
function addItemCart(){
  if (data?.product?.attributes == false){
    ...
    }else if ( data?.product?.attributes.length > 0 && data?.product?.attributes.length === cartAttributes.length){
         setCartItem(prevCartItem=>[...prevCartItem,obj]);
         setQuantity(prev => prev + 1);
    }else{
  ...
  <div className="parent-cont">
    <div className="product-page-grid">
      ...
      <div className="product-description">
        <div>
          <h1 style={{width: "70%",fontWeight:"600"}}>{name}<span></span></h1>
          <h1 style={{width: "70%",fontWeight:"400",marginTop:"-15px"}}>{bottomname}<span></span></h1>
        </div>

{/* karoche aq mogvarebulia atributebis darendereba */}
      <div>
        {data?.product.attributes?.map(prev =>{
          return (
           <ul >
              <h4 style={{fontFamily:"Roboto",textTransform:"uppercase"}}>{prev.name} {prev.name? <span>:</span> : null}</h4>
            <div style={{display: "flex", flexDirection:"row"}}>
                ...
                const modifyAttributes = [...cartAttributes];
                  ...
                  <li
                       onClick={()=> {
                       const sameItem = modifyAttributes.find(atr=> atr.name === prev.name);
                       const attributesToAdd = {name: prev.name, displayValue: item.displayValue};
                       const sameItemIndex = modifyAttributes.indexOf(sameItem);

                       sameItem ? modifyAttributes.splice(sameItemIndex,1,attributesToAdd) : modifyAttributes.push(attributesToAdd);
                       setCartAttributes(modifyAttributes);
                      }}
                      style={prev.name === "Color" ? {backgroundColor:item.id} : null }
                  >
                      {prev.name === "Color" ?  null
                      : <span style={{fontFamily:"Roboto"}}>{RenderAttr(item)}</span>}
                  </li>
                       )
               })}
             </div>
...
{/* karoche aq mogvarebulia atributebis darendereba */}
    <div>
        <h4 style={{maxWidth:"63px", margin:"5px", background:"red", display: "inline"}}>{attributevalue}</h4>
    </div>
    <div style={{marginBottom:"10px",fontFamily:"Roboto"}}>
      <strong>PRICE:</strong>
    </div>
    <div style={{marginBottom:"20px"}}>
      <span><h2 style={{display:"inline"}}>{data?.product.prices[currencyIndex].currency.symbol}</h2></span>
      <span><h2 style={{display:"inline"}}>{data?.product.prices[currencyIndex].amount}</h2></span>
    </div>
    ...
    <div style={{marginBottom:"40px"}}>
      <button onClick = {addItemCart}  className="add-to-cart-btn">
        ...
        <h3 style={{fontFamily:"Roboto",fontWeight:"400"}}>{description}</h3>
